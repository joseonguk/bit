...
public class CopyOfScoreList01 extends HttpServlet{
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    // 출력되는 내용을 지정된 문자 집합으로 인코딩한다.
    // 인코딩 -> 문자집합 명세서에 나와 있는대로 각 문자에 대해 코드 값으로 바꾼다.
    // 'A' -> 0x41(1bytr), '가' => 0x80xxxx (3byte)
    ...
    out.println("  <form action='/web01/score/form.html'><input type='submit' value='추가' ></from>");
    out.println("  <table>");
    out.println("   <tr>");
    ...
    try{
      ...
      for(Score score : scores){
        out.println("  <tr>");
        out.format("    <td><a href='updateform?no=%1$d'>%2$d</a></td>", score.getNo(), score.getNo());
