 ...
 * 		super 클래스의 메서드는 시그너처(signature)와 같게 선언해야 한다.
 * - 메서드 시그너처 => c에서는 function prototype
 * 	  이름, 리턴타입, 파라미터 타입
 * 
 * - super 변수
 * 		오버라라이딩 전에 변수나 메서드를 가리키는 레퍼런스
 */
...
public class CalculatorEnt extends CalculatorPro {
	...
	public void plus(String value) {
	  //String removedSpace = removeMonetaryUnitChar(value.trim());		// 2번
	  
	  //String temp = removeMonetaryUnitChar(value.trim());		// 1번
	  
	  // super.오버라이딩하기 전에 메서드명()
	  super.plus(removeMonetaryUnitChar(value.trim()));			// 3번
	}
